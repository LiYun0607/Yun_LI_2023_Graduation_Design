...
from models_cpo_continous import Policy
from models_cpo_continous import Value
...
class Agent:
    def __init__(self, env, device, args):
        ...
        # member variable
        self.save_dir = '/home/zr/下载/CARLA_0.9.12_RSS/PythonAPI/examples/result/0424.pt'
        ...
        self.v_scheduler = torch.optim.lr_scheduler.LambdaLR(self.v_optimizer, verbose=True, lr_lambda=lambda epoch: (1-epoch/self.epoch_n))
        self.cost_v_scheduler = torch.optim.lr_scheduler.LambdaLR(self.cost_v_optimizer, verbose=True, lr_lambda=lambda epoch: (1-epoch/self.epoch_n))
        self.i = 0
        self.j = 0
        # self.load()
    ...
    #         clipped_action:  Tensor(action_dim,)
    #     '''
    #     # print(self.cost_value(state))
    #     mean, std = self.policy(state)
    #     # if is_train:
    #     #     noise = torch.randn(*mean.size(), device=self.device)
    #     #     print("noise", noise)
    #     #     action = self.normalizeAction(mean + noise*std)
    #     # else:
    #     action = self.unnormalizeAction(mean+std)
    ...
    def getAction(self, state:torch.Tensor, is_train:bool) -> List[torch.Tensor]:
        ...
        mean, std = self.policy(state)
        action_dim = torch.tensor(self.action_dim)
        action_std = torch.ones_like(action_dim)*std
        mean = mean.to(self.device)
        action_std = action_std.to(self.device)
        mean = self.unnormalizeAction(mean)
        distribution = Normal(mean, action_std)
        action = distribution.sample()
        clipped_action = clip(action, self.action_bound_max, self.action_bound_min)
        # print(action, clipped_action, action_std)
        return action, clipped_action
    ...
    def train(self, trajs):
        ...
        # ========== for policy update ========== #
        # backup old policy
        means, stds = self.policy(states_tensor)
        print("std:", stds)
        old_means = means.clone().detach()
        old_stds = stds.clone().detach()

        # get objective & KL & cost surrogate
        norm_actions_tensor = norm_actions_tensor.to(self.device)
        old_means = old_means.to(self.device)
        old_stds = old_stds.to(self.device)
        ...
        # print(self.value(states_tensor))
        while True:
        ...
        for _ in range(self.value_epochs):
            ...
            # print("vl. vl",value_loss, cost_value_loss)
        # ======================================== #
        ...
        #     self.j = 0
        # if self.i >= 5:
        #     self.cost_d = self.cost_d * 0.8
        #     self.i = 0
        #     print("reduce cost_d as finding good boundary, cost_d now is:", self.cost_d)
        # if optim_case < 2:
        #     self.j += 1
        #     self.i = 0
        # if self.j >= 30:
        #     self.cost_d = self.cost_d * 1.2
        #     self.j = 0
        #     print("increase cost_d as not finding good boundary, cost_d now is:", self.cost_d)

        self.v_scheduler.step()
        self.cost_v_scheduler.step()
        return np_value_loss, np_cost_value_loss, np_objective, np_cost_surrogate, np_kl, np_entropy, grad_g, grad_b, optim_case
