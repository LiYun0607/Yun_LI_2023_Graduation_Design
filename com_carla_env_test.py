...
class CarlaEnv(gym.Env):
    ...
    def step(self, action):
        ...
        # Calculate the total amount of data generated in this time step
        data_generated = sum(d["data amount"] for d in self.data_all)
        ...
        # Add the throughput in this time step to the total throughput
        self.throughput += throughput_this_step

        # Calculate PER for this time step
        PER_this_step = data_loss_total / data_generated if data_generated > 0 else 0

        # Calculate spectral efficiency for this time step
        bandwidth = 1e7  # replace with your system's bandwidth in Hz
        spectral_efficiency_this_step = data_transmitted / (
        ...
        for i in range(self.n_vehicles):
            if action[i] == 0 or action[i] == 1:  # V2I channel
                energy_consumed_this_step[i] = v2i_power_mW * 0.1
            elif action[i] == 2:  # V2V channel
                energy_consumed_this_step[i] = v2v_power_mW * 0.1
            # no energy consumed if action[i] == 3 (no channel)

        # Calculate total energy consumed
        total_energy_consumed = np.sum(energy_consumed_this_step)

        # Calculate energy efficiency for this time step
        energy_efficiency_this_step = data_transmitted / total_energy_consumed if total_energy_consumed > 0 else 0
        fairness_index_history = self.calculate_fairness_index()

        # You might want to store these values for later analysis
        self.PER_history.append(PER_this_step)
        self.throughput_history.append(throughput_this_step)
        self.spectral_efficiency_history.append(spectral_efficiency_this_step)
        self.energy_efficiency_history.append(energy_efficiency_this_step)
        self.fairness_index_history.append(fairness_index_history)
        ...
        if self.time_step >= args.number_of_episodes:
            ...
            # Calculate PER
            total_data = sum([data['data amount'] for data in self.data_all])
            erroneous_data = sum(self.lost_data_all)
            PER = erroneous_data / total_data if total_data > 0 else 0
            ...
            df = pd.DataFrame({
                ...
                'spectral_efficiency': self.spectral_efficiency_history,
